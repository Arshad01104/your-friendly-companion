...
const ChatHeader: FC<ChatHeaderProps> = ({ name, status, avatarUrl }) => {
  const statusText = {
    online: "Online",
    typing: "Typing...",
    offline: "Offline",
  ...
  return (
    <header className="flex items-center gap-4 px-6 py-4 bg-card/80 backdrop-blur-md border-b border-border/50 shadow-soft">
      <div className="relative">
        <div className="w-12 h-12 rounded-full gradient-warm shadow-glow flex items-center justify-center overflow-hidden">
          {avatarUrl ? (
        ...
        <span
          className={`absolute bottom-0 right-0 w-3.5 h-3.5 ${statusColor[status]} rounded-full border-2 border-card ${
            status === "typing" ? "animate-pulse-soft" : ""
          }`}
        />
      </div>
